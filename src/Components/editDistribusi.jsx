...
export default function PostEdit() {
  ...
  const fetchDetailPost = async () => {
    //fetch data
    await api.get(`/api/registrasi/${id}`).then((response) => {
      //assign to state
  ...
  const updatePost = async (e) => {
    ...
    //send data with API
    await api
      .post(`/api/registrasi/${id}`, formData)
      .then(() => {
      ...
      <div className="row">
        <form onSubmit={updatePost}>
          ...
          <div className="mb-3 bg-coklat-kuning ">
            ...
            ></textarea>
            {errors.role && (
              <div className="alert alert-danger mt-2">{errors.role[0]}</div>
            )}
          </div>
